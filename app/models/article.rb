...
class Article < ApplicationRecord
  ...
  def match_article_input(article_name)
    ...
    if match_data.present? && split_article_name(article_name).size >= 4
  ...
  def process_unmatched_input(article_name)
    split_article = split_article_name(article_name)
  ...
  def split_article_name(article_name)
    article_name.gsub(/\s+/m, ' ').strip.split(' ')
  end
end
